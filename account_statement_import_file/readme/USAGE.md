...
Duplicate prevention depends on the file parser supplying a stable, non-empty
transaction identifier. Transactions with an identifier already imported in
Odoo are skipped. Transactions without one can be created again, so check
for earlier imports before uploading the same file. For spreadsheet imports,
map the unique transaction ID column when the source provides one.
